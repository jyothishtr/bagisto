...
class CartTokenProcessor implements ProcessorInterface
{
     ...
     * mutation input to CartInput reliably.
     */
    private function hydrateInputFromArgs(CartInput $data, mixed $args): void
    {
        ...
        $fieldAliases = [
            ...
            'shippingAddressId'   => ['shippingAddressId', 'shipping_address_id'],
            'billingAddressId'    => ['billingAddressId', 'billing_address_id'],
            'shippingMethod'      => ['shippingMethod', 'shipping_method'],
            'selectedShippingRate'=> ['selectedShippingRate', 'selected_shipping_rate'],
            'paymentMethod'       => ['paymentMethod', 'payment_method'],
            'sessionId'           => ['sessionId', 'session_id'],
            'createNew'           => ['createNew', 'create_new'],
            'itemIds'             => ['itemIds', 'item_ids'],
        ...
        foreach ($fieldAliases as $property => $aliases) {
            ...
            $data->{$property} = $this->castInputValue($property, $value);
        }
    }
    ...
    /**
     * Cast values from GraphQL args into CartInput property types.
     */
    private function castInputValue(string $property, mixed $value): mixed
    {
        return match ($property) {
            'id', 'couponCode', 'shippingMethod', 'paymentMethod', 'sessionId', 'country', 'state', 'postcode'
                => is_scalar($value) ? (string) $value : null,

            'createNew'
                => filter_var($value, FILTER_VALIDATE_BOOLEAN, FILTER_NULL_ON_FAILURE) ?? false,

            'cartId', 'productId', 'cartItemId', 'quantity', 'shippingAddressId', 'billingAddressId'
                => is_numeric($value) ? (int) $value : null,

            'options', 'itemIds', 'quantities'
                => is_array($value) ? $value : $this->normalizeArgs($value),
